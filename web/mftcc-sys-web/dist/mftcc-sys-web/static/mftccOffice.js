 ...
 *
 * Version: 6.0.0 (build:0)
 */

let DocsAPI = {
  DocEditor: function (placeholderId, config) {
    var _self = this,
      _config = config || {};

    extend(_config, DocsAPI.defaultConfig);
    ...
    if (target && _checkConfigParams()) {
      ...
      var _msgDispatcher = DocsAPI.MessageDispatcher(_onMessage, this);
    }
...
function getExtensionPath (url) {
  if (url) {
    return url + "/web-apps/apps/"
  } else {
    return null;
  }
  // let url = "http://192.168.110.161/web-apps/apps/"
  // http://192.168.110.161/web-apps/apps/
...
function getAppPath (config) {
  var extensionPath = getExtensionPath(config.url),
