    ...
    <el-main>
      <div class="app-content">
        ...
        <el-drawer
          ...
          :with-header="false"
        >
          <el-row v-if="appFormId != '' && appFormShow" class="appFormRow">
            <mftcc-form
              :formId="appFormId"
              :parentVm="this"
              ref="appBizForm"
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="配置" prop="type" v-show="listBtnShow">
                  <el-button type="primary" @click="showNodeList"
                    >配置</el-button
                  >
                </el-form-item>
              </el-col>
...
export default {
  ...
  props: ["appData", "query", "customNodeList"],
  watch: {
    approveShow(val) {
      if (val === true) {
  ...
  methods: {
    ...
    userDialogGetAllCallback(value) {
      ...
      this.$refs.nodeList.setFormValue(
        this.clickIndex,
        "assigneeId",
        fieldValue
      );
    ...
    saveNode() {
      this.listShow = false;
      this.addNodeList = this.$refs.nodeList.getFormValue().allData;
    },
    showNodeList() {
      if (this.listShow) {
        this.listShow = false;
    ...
    selectType() {
      ...
      if (
      ...
      } else if (approveBtnSub.approveType == 2) {
        this.nodeShow = false;
        this.$nextTick(() => {
          this.nodeShow = true;
        });
        ...
        approveBtnSub.approveType == 7 ||
        approveBtnSub.approveType == 8
      ) {
        this.textShow = false;
        this.nodeShow = false;
      ...
      if (node.needAssign === true) {
        if (node.assignList == "getAll") {
          ...
          this.userOption = {
            ...
            isPagination: true,
            url: "/mftcc-flowable-server/user/pageByUserId"
          };
      ...
      if (this.approveShow) {
        ...
        if (this.appFormId != "") {
          if (this.appFormInitMethod) {
            if (
              typeof this.$refs.component[this.appFormInitMethod] == "function"
            ) {
        ...
        if (valid === true) {
          this.$confirm("此操作将提交该笔业务, 是否继续?", "提示", {
            ...
            if (this.variablesMethod) {
              if (
                typeof this.$refs.component[this.variablesMethod] == "function"
              ) {
                let formData = {};
...
.appDetails .appFormRow {
  margin-bottom: 22px;
  height: calc(100% - 160px);
  overflow: auto;
  border-bottom: 1px solid #f1f1f1;
}
.appDetails .appFormRow2 {
  text-align: left;
  height: 160px;
  padding: 10px;
}
.appDetails .el-drawer__wrapper {
  position: absolute;
}
.appDetails .app-content {
  height: 100%;
  position: relative;
}
.appDetails .el-footer {
  background: #f1f8ff;
}
.appDetails .el-footer .app-footer {
  margin: 0 auto;
}
</style>
