<template>
    <div class="mftcc-container">
        ...
        <el-container>
            <el-row>
                <mftcc-form formId="example/sysViewUpdateForm" :parentVm="this" ref="sysViewUpdateForm"></mftcc-form>
...
export default {
    ...
    methods: {
        submitForm() {
            this.$refs.sysViewUpdateForm.validateForm((valid) => {
                if (valid) {
                    let data = this.$refs.sysViewUpdateForm.getFormValue();
                    api.update(data,res => {
                        if(res.code === 0){
                            this.$message({
                                type: res.msgType,
        ...
        findById(){
            ...
            api.findById(viewId,res => {
                if(res.code === 0){
                    var data = res.data;
                    this.$refs.sysViewUpdateForm.setFormValue(data);
                }else{
                    this.$alert(res.msg, this.$alertTitle, {
                        type: res.msgType,
                        dangerouslyUseHTMLString: true,
