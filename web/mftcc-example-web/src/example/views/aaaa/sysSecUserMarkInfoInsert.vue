<template>
    <div class="mftcc-container">
        ...
        <el-container>
            <el-row>
                <mftcc-form formId="example/sysSecUserMarkInfoInsertForm" :parentVm="this" ref="sysSecUserMarkInfoInsertForm"></mftcc-form>
...
export default {
    ...
    methods: {
        submitForm() {
            this.$refs.sysSecUserMarkInfoInsertForm.validateForm((valid) => {
                if (valid) {
                    let data = this.$refs.sysSecUserMarkInfoInsertForm.getFormValue();
                    api.insert(data,res => {
                        if(res.code === 0){
                            this.$message({
                                type: res.msgType,
