  ...
  return {
    execute: function () {
      ...
      function navigateToUrl (obj) {
        ...
        if (typeof obj === "string") {
          ...
          if (urlArr.length == 2) {
            ...
            for (var i = 0; i < pArr.length; i++) {
              var queryArr = pArr[i].split("=");
              var queryStr = queryArr[0] + "=" + encodeURIComponent(queryArr[1]);
              str += queryStr;
              if (i < pArr.length - 1) {
                str += "&";
              }
        ...
        if (url.indexOf("#") === 0) {
          ...
          if (paramArr.length > 1) {
            var urlStr = paramArr[0];
            var paramStr = paramArr[1];
            paramStr = paramStr.replace(/\//g, "%2F");
            url = urlStr + "?" + paramStr;
          } else {
      ...
      function getApp (name) {
        return appsNew[name] || null;
      }
      function getProJect (productName, success, error, flag) {
        var app = find(apps, function (App) {
          return toName(App) === productName;
        });
        if (flag) {
          System.delete(System.resolve(productName));
        }
        toLoadPromise(app)
          .then(res => {
      ...
      function registerApplication (
        ...
        appsNew[registration.name] = assign(
          {
            loadErrorTime: null,
            status: NOT_LOADED,
            parcels: {},
      ...
      function reroute () {
        ...
        function performAppChanges () {
          return Promise.resolve().then(function () {
            ...
            var allUnmountPromises = unmountUnloadPromises.concat(
              unloadPromises
            );
      ...
      if (isInBrowser) {
        window.__SINGLE_SPA_DEVTOOLS__ = devtools;
      }
    }
  };
});
